...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState, costFn = lambda x: 1):
    ...
    "*** Your Code Here ***"
    self.costFn = costFn 
    self.Id = self.startingPosition #Id for what goal has been discovered
    self.count = 0
    self.vis = []
  ...
  def isGoal(self, state):
    ...
    if state[0] in self.corners:
      if not state[0] in self.vis:
        self.Id = state[0]
        self.count += 1
        self.vis.append(state[0])
    if self.count == len(self.corners):
      return True
  ...
  def successorStates(self, state):
    ...
    Returns successor states, the actions they require, and a cost of 1.
     ...
     cost of expanding to that successor
    """
    #turn start state into a tuple of coord, and Id
    if state == self.startingPosition and self.count == 0:
      state = (state, self.startingPosition)
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      "*** Your Code Here ***"
      ((x,y), corners) = (state[0], self.Id) #tuple of coord and Id
      dx, dy = Actions.directionToVector(action) #
      nextx = int(x+dx)
      nexty = int(y+dy)
      ...
      if not self.walls[nextx][nexty]:
        nextState = (nextx, nexty)
        cost = self.costFn(nextState)
        succ.append(((nextState, self.Id), action, cost))
