  ...
  def minimaxDecision(self, state):
    ...
    for action in state.getLegalActions(0):
      if action != Directions.STOP:
        bestValue = v
        # finds argmax_a
        v = max(v, self.minValue(0, state.generateSuccessor(0, action), 1))
...
class AlphaBetaAgent(MultiAgentSearchAgent):
  ...
  def alphaBetaSearch(self, state):
    ...
    for action in state.getLegalActions(0):
      if action != Directions.STOP:
        bestValue = v
        # finds argmax_a
        v = max(v, self.abMinValue(0, state.generateSuccessor(0, action), 1, -1*float("inf"),float("inf")))
        #compares previous utility value and extracts the action of max v
        if v > bestValue:
          bestMove = action
  ...
  def abMaxValue(self, depth, state, a, b):
    if depth == self.treeDepth:
      return scoreEvaluationFunction(state)
    v = -1*float("inf")
    for action in state.getLegalActions(0):
      if action != Directions.STOP: #removes Directions.STOP
        v = max(v, self.abMinValue(depth, state.generateSuccessor(0, action), 1, a, b))
        if v >= b:
          return v
        else:
          a = max(a,v)
  ...
  def abMinValue(self, depth, state, ghostCount, a, b):
    if depth == self.treeDepth:
      return scoreEvaluationFunction(state)
    v = float("inf")
    for action in state.getLegalActions(ghostCount):
      if action != Directions.STOP: #removes Directions.STOP
        if ghostCount == state.getNumAgents()-1:
          v = min(v, self.abMaxValue(depth+1, state.generateSuccessor(ghostCount, action), a, b))
        else:
          #if not all ghosts have made a move
          #treat all min levels between previous max and the next max as the same depth
          v = min(v, self.abMinValue(depth, state.generateSuccessor(ghostCount, action), ghostCount+1, a, b))
    return v
...
class ExpectimaxAgent(MultiAgentSearchAgent):
  ...
  def value(self, state):
    ...
    for action in state.getLegalActions(0):
      if action != Directions.STOP:
        bestValue = v
        v = max(v, self.expValue(1, state.generateSuccessor(0, action), 1))
        #compares previous utility value and extracts the action of max v
        if v > bestValue:
          bestMove = action
    return bestMove
  ...
  def maxValue(self, depth, state):
    ...
    for action in state.getLegalActions(0):
      if action != Directions.STOP: #removes Directions.STOP
        v = max(v, self.expValue(depth, state.generateSuccessor(0, action), 1))
    return v
  
  def expValue(self, depth, state, ghostCount):
    if depth == self.treeDepth:
      return self.evaluationFunction(state)
    v = 0
    numLegalActions = len(state.getLegalActions(ghostCount))
    for action in state.getLegalActions(ghostCount):
      if action != Directions.STOP: #removes Directions.STOP
        #print state.getLegalActions(ghostCount)
        if ghostCount == state.getNumAgents()-1:
          v += self.maxValue(depth+1, state.generateSuccessor(ghostCount, action))/numLegalActions
        else:
          #if not all ghosts have made a move
          #treat all min levels between previous max and the next max as the same depth
          v += self.expValue(depth, state.generateSuccessor(ghostCount, action), ghostCount+1)/numLegalActions
    return v
    util.raiseNotDefined()
...
def betterEvaluationFunction(currentGameState):
  """
    ...
    DESCRIPTION:    Tried making an evaluation from Q1 but using information on current states. 
  ...
  #subroutine for finding the nearest food
  for food in nextFoodLocations:
    foodList.append(util.manhattanDistance(food, position))
  fDist = min(foodList)
    
  ghostPosition = ghostStates[0].getPosition()
  gDist = util.manhattanDistance(position, ghostPosition)
  ...
  return prioritize + currentGameState.getScore()
  util.raiseNotDefined()
